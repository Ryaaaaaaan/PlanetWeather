...
struct CosmicNavigationRoot: View {
    ...
    var body: some View {
        ZStack {
            // Couche 1 : SolarSystemOrreryView (La 3D interactive)
            SolarSystemOrreryView(selectedPlanet: $selectedPlanet)
                .ignoresSafeArea()
            
            // Couche 2 : PlanetInfoOverlay (L'UI native en verre)
            PlanetInfoOverlay(selectedPlanet: $selectedPlanet)
        }
    }
}

// MARK: - Solar System Orrery View (3D Scene)
// Consolidated here to ensure compilation target visibility
...
struct SolarSystemOrreryView: View {
    ...
    private func createOrbitRing(radius: Float) -> Entity {
        ...
        // RealityKit opacity expects explicit initialization or literal
        material.blending = .transparent(opacity: .init(0.2))
    ...
    private func createPlanetEntity(for planet: Planet, scale: Float) -> Entity? {
        ...
        if planet.hasRings {
            ...
            ringMat.color = .init(tint: UIColor.white.withAlphaComponent(0.4))
            ringMat.blending = .transparent(opacity: .init(0.4))
...
// MARK: - Planet Info Overlay (Native UI)
// Consolidated here to ensure compilation target visibility
